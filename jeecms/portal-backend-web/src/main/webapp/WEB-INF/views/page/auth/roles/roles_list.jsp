                            ...
                            <div>
                                ...
                                <form id="queryForm" class="form-inline">
                                    <div class="form-group">
                                        <label class="control-label sr-only">角色名：</label>
                                        <input name="name" type="text" placeholder="请输入角色名" class="form-control">
                                    </div>
                                    <div class="form-group">
                                        <label class="control-label sr-only">是否可用：</label>
                                        <select name="isok" class="form-control">
                                            <option value="">--请选择 是否可用--</option>
                                            <option value="1">可用</option>
                                            <option value="0">不可用</option>
                                        </select>
                                    </div>
                                    <div class="form-group">
                                        <button id="queryBtn" class="btn  btn-primary" type="button" style="margin-top: 6px;">查询</button>
                                    </div>
                                </form>
                                <%--工具栏--%>
                                <div id="tableToolbar" class="btn-group hidden-xs" role="group">
                                    <shiro:hasPermission name="auth_role:2">
                                        <button id="addBtn" type="button" class="btn btn-outline btn-default">
                                            <i class="glyphicon glyphicon-plus" aria-hidden="true"></i>
                                        </button>
                                    </shiro:hasPermission>
                                    <shiro:hasPermission name="auth_role:8">
                                        <button id="removeBtn" type="button" class="btn btn-outline btn-default" disabled>
                                            <i class="glyphicon glyphicon-trash" aria-hidden="true"></i>
                                        </button>
                                    </shiro:hasPermission>
                                </div>
                                <table id="roleTable" class="table table-hover" data-striped="true"></table>
                            </div>
                            <!-- End Example Events -->
                        </div>
                ...
                columns: [
                    ...
                    , {
                        ...
                        formatter: function (value, row, index) {
                            ...
                            <shiro:hasPermission name="auth_role:4">
                            ...
                            <shiro:hasPermission name="auth_role:8">
                                ...
                                array.push('<a class="role" href="javascript:void(0)" title="菜单"><i class="glyphicon glyphicon-user"></i></a>');
                            return array.join('');
                        },
                        events: {
                            ...
                            'click .role': function (e, value, row, index) {
                                auth_global.commonLayer({
                                    title:"菜单权限分配",
                                    content:"${ctx}/roles/selectFuncTreePage?roleId="+row.id
                                });
                            }
                        }
                    }]
